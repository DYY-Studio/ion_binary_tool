...
class IonText(IonSerial):
    ...
    def serialize_symbol_value(self, value):
        ...
        return "'%s'" % escape_string(result, quote="'", escape_unicode=self.escape_unicode)
    ...
    def serialize_string_value(self, value):
        return "\"%s\"" % escape_string(value, quote="\"", escape_unicode=self.escape_unicode)
...
def escape_string(s, quote="\"", escape_unicode=True):
    ss = []
    i = 0
    while i < len(s):
        c = s[i]
        ...
        if c == quote or c == "\\":
            ...
            ss.append("\\r")
        elif o > 0x7f and not escape_unicode:
            ss.append(c)
        elif o <= 0xff:
            ss.append("\\x%02x" % o)
